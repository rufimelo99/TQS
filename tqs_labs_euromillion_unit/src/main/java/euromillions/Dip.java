...
public class Dip {
    ...
    public Dip(int[] arrayOfNumbers, int[] arrayOfStarts) {
        ...
        for(int i = 0; i<arrayOfNumbers.length; i++){
            if(arrayOfNumbers[i]<0 || arrayOfNumbers[i]> 60 ){
                throw new IllegalArgumentException("wrong number inserted");
            }
        }
        for(int i = 0; i<arrayOfStarts.length; i++) {
            if (arrayOfStarts[i] < 0 || arrayOfStarts[i] > 6) {
                throw new IllegalArgumentException("wrong star inserted");
            }
        }
